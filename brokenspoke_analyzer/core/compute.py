...
def conectivity(
    ...
    result = dbcore.execute_query_with_result(engine, "select road_id FROM neighborhood_ways")
    ...
    for stress_level in ["high", "low"]:
        ...
        with concurrent.futures.ThreadPoolExecutor() as executor:
            future_to_road_id = {executor.submit(execute_sqlfile_with_substitutions, engine, sql_script, { "road_id": i, "nb_max_trip_distance": max_trip_distance}, 0.01): i for i in road_ids}
    ...
    # Worth calculating blockids without population?
    # add WHERE pop10 > 0?
    # Fetch all blockids and then calculate stress for them individually but in parallel
    result = dbcore.execute_query_with_result(engine, "select blockid10 FROM neighborhood_census_blocks")
    census_block_ids = [e for l in result for e in l]
    census_block_ids.sort()

    with concurrent.futures.ThreadPoolExecutor() as executor:
        future_to_census_block_id = {executor.submit(execute_sqlfile_with_substitutions, engine, sql_script, { "nb_max_trip_distance": max_trip_distance, "nb_output_srid": output_srid, "block_id": f"'{i}'" }, 0.01): i for i in census_block_ids}
