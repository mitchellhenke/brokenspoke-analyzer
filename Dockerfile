...
RUN pip install uv \
  && uv export --format requirements-txt --all-extras --no-group dev --no-hashes --no-emit-project -o requirements.txt \
  && mkdir -p deps \
...
#
# Install dependency wheels once, cached independently
#
FROM base AS deps-installed
WORKDIR /usr/src/app
COPY --from=deps /usr/src/app/deps ./pkg/deps
